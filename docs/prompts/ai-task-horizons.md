...
!!! prompt
    Please use the chartjs-generator skill to create a similar plot using a microSim.  
    Try to use the exact same datapoint as this chart.  
    Use very small margins so that the chart can easily be inserted into any mkdocs page using an iframe 
